...
.center[
<img src="Figures/languages.png" alt="Languages" style="height:480px;">
]
...
.center[
<img src="Figures/frameworks.png" alt="Frameworks" style="height:480px;">
]
...
---
## Gorin, Heblich & Zylberberg — State of the Art

.small123[
NBER working paper: [www.nber.org/system/files/working_papers/w33976/w33976.pdf](https://www.nber.org/system/files/working_papers/w33976/w33976.pdf) — Paper website: [state.arteconlab.org](https://state.arteconlab.org/)
]

.panelset[
.panel[.panel-name[Abstract]

.center[
<img src="Figures/Gorin, Heblich, Zylbergberg - State of the Art Abstract.png" alt="Abstract" style="height:420px;">
]
...
.center[
<img src="Figures/Gorin, Heblich, Zylbergberg - State of the Art Figure3.png" alt="Figure 3" style="height:420px;">
]

]
.panel[.panel-name[Figure 6]

.center[
<img src="Figures/Gorin, Heblich, Zylbergberg - State of the Art Figure6.png" alt="Figure 6" style="height:420px;">
]
...
.pull-right-narrow[
.tiny123[
*"As an econometrician, my answer is that while it's possible to conduct economic history research without advanced statistical techniques, the results may be less accurate and reliable. Historical data is often incomplete, noisy, and subject to various forms of uncertainty, which can make it challenging to draw meaningful conclusions without the use of advanced statistical methods. By applying advanced techniques like time series analysis, panel data models, and Bayesian methods, we can better account for these sources of uncertainty..."*
]
]
...
```{r load-stockholm, echo = F, cache.extra = file.mtime("Debiasing_exercises/Stockholm1920.csv")}
...
```{r, echo = F}
table(predicted = stockholm$female_pred_binary, actual = stockholm$female)
```

---
# .red[Exercise: Gender wage gap]

Load in the Stockholm wage data from the file `Stockholm1920.csv`. Then:
...
2. Regress `log_labor_income` on `female_pred_binary` using **all** observations. What is the estimated gender wage gap?
...
**Hint**: Consider using this Colab notebook to get started: https://colab.research.google.com/drive/1oU5c9SZ9geUFAxU4hfPMU7SoN7D66No-?usp=sharing

```{r exercise-gap-solution, echo = FALSE, eval = FALSE}
cm <- table(predicted = stockholm$female_pred_binary, actual = stockholm$female)
c(accuracy = sum(diag(cm))/sum(cm), precision = cm[2,2]/sum(cm[2,]), recall = cm[2,2]/sum(cm[,2]), f1 = 2*cm[2,2]/(2*cm[2,2]+cm[2,1]+cm[1,2]))

summary(lm(log_labor_income ~ female_pred_binary, data = stockholm))
...
naive <- b(lm(log_labor_income ~ female_pred_binary, stockholm))
...
We can now combine this bias correction with our full set of ML predictions
...
The two most prominent methods for debiasing are **prediction-powered inference** (PPI; Angelopoulos et al., 2023) and design-based supervised learning (DSL; Egami et al., 2023, 2024).
...
**Hint**: Consider using this Colab notebook to get started: https://colab.research.google.com/drive/1vqQLGcDtViuZhcm6GmytBxexqHdK-9Vj?usp=sharing
...
Let us generate a single draw according to the previous exercise's DGP

```{python ppi-setup, cache = FALSE}
import numpy as np
...
```{python ppi-estimate, comment = "", cache = FALSE}
Y_gs = Y[R == 1]
Y_pred_gs = Y_pred[R == 1]
Y_pred_ml = Y_pred[R == 0]

theta = ppi_mean_pointestimate(Y_gs, Y_pred_gs, Y_pred_ml)
lo, hi = ppi_mean_ci(Y_gs, Y_pred_gs, Y_pred_ml, alpha = 0.05)
theta, lo, hi = [np.asarray(v).item() for v in (theta, lo, hi)]

print(f"PPI: {theta:.3f}  [{lo:.3f}, {hi:.3f}]")
print(f"Naive: {Y_pred.mean():.3f}")
print(f"Gold: {Y_gs.mean():.3f}")
```
...
.pull-right-narrow[
.small123[
200 paragraphs of real seventeenth-century English (EEBO), labelled by a frontier LLM (ChatGPT 5.5) and, independently, by us
...
Load in the Stockholm wage data from the file `Stockholm1920.csv`. Then:
1. Use DSL as shown previously to obtain a debiased version of the gender wage gap estimate
1. Compare with the results from the previous exercise. Which is the best estimate?

**Hint**: Consider using this Colab notebook to get started: https://colab.research.google.com/drive/1LI1uYDJwrjYXGAs0FekzbvG8tlXN44Zy?usp=sharing

```{python stockholm-dsl-py, comment = "", cache = FALSE, echo = FALSE, eval = FALSE}
import numpy as np, pandas as pd

d = pd.read_csv("Debiasing_exercises/Stockholm1920.csv")

def dsl_gap(df):
    Y = df["log_labor_income"].to_numpy()
    Z_pred = df["female_pred"].to_numpy(float)
    R = df["R"].to_numpy(float)
    Z = df["female"].fillna(0).to_numpy(float)
    pi = R.mean()

    Zt = Z_pred + R / pi * (Z - Z_pred)           # the pseudo outcomes
    Zt2 = Z_pred**2  + R / pi * (Z**2 - Z_pred**2)
    ZtY = Z_pred * Y + R / pi * (Z * Y - Z_pred * Y)

    return (ZtY.mean() - Zt.mean() * Y.mean()) / (Zt2.mean() - Zt.mean()**2)

print(f"DSL gap: {dsl_gap(d):.3f}")
```

---
## Using DSL to correct our estimate

.center[
<img src="Figures/stockholm_dsl_coefplot.png" alt="Gender wage gap estimates, Stockholm 1920" style="width:92%;">
]
...
2. Standard performance scores do not detect the resulting bias in your estimate
3. A small **random** sample of hand-coded "gold standard" labels lets you measure the bias and correct it
4. Validity comes from the **sampling design** - the ML model only buys precision
5. This covers much of what economic history does with ML

---
# Next time

.pull-left[
**Lecture 2: From tokens to transcription** - we go under the hood and build our own models

- Neural networks for text, and building a text classifier from scratch (OccCANINE)
- Transcription of scanned documents: handwritten text recognition and LLMs for OCR
]
