...
- ... but our estimate using only observed gender is measured imprecisely
...
.pull-left-wide[
$N$ observations. For each we have features $X_i$ and a prediction $f(X_i)$.

For a random subset of $n \ll N$ we also paid someone to establish the truth $Y_i$. Let $R_i = 1$ for those.
]

--

.pull-left-wide[
In the Stockholm data, $N = 3{,}271$ and $n = 330$.

Start with the simplest possible target, the mean $\theta = \mathbb{E}[Y_i]$.
]
...
.pull-left-wide[
And biased by exactly the average prediction error, which we have no reason to believe is zero.
]
...
So we can simply measure the average error:
...
---
## Option 3: predictions, corrected

.pull-left-wide[
$$\hat{\theta}_{DB} = \frac{1}{N-n}\sum_{i=n+1}^{N} f(X_i) \;-\; \frac{1}{n}\sum_{i=1}^{n}\big(f(X_i) - Y_i\big)$$
]

--

.pull-left-wide[
- **First term:** average the predictions over the unlabelled sample
- **Second term:** subtract the error you measured on the labelled one
]

--

.pull-left-wide[
Two names for this in the literature: **prediction-powered inference** (PPI) and **design-based supervised learning** (DSL).
]
...
The variance depends on how much the prediction errors vary, not on how much $Y$ varies. A good model makes the correction small and stable.
]

--

.pull-left-wide[
A weak model costs you precision, never unbiasedness.
]

---
# .red[Exercise 2: Does it actually work?]

.pull-left-wide[
Simulated data, where you know the answer. Let $X_i \sim U(0,1)$ and $Y_i = 2X_i$, so the
true mean is $\theta = 1$. Use the deliberately imperfect predictor
$f(X_i) = 2.1 X_i - 0.5 X_i^2$.

Take $N = 1{,}000$ and label a random $n = 100$.

1. Compute all three estimators from the last few slides
2. Repeat 1,000 times and plot the three distributions
3. What share of your **naive** 95% intervals contain the truth?
]
...
.pull-left-wide[
- **Convenience samples.** Coding the first 300 records is not a design.
...
- **A biased source.** This corrects the model, not the archive.
]

---
## The labels are the hard part

.pull-left-wide[
The correction is only as good as the $n$ labels behind it.

Ambiguity rises from **numbers**, through **occupations**, to **semantics**. At the left end the task is hard for the model and trivial for a human; at the right end it needs someone who knows the period.
]
...
.pull-left-wide[
The same logic reaches much further:

- Predictions on the **right-hand side** of a regression, which is the Stockholm case
- Regression coefficients generally: linear, logistic, Poisson
- High-dimensional fixed effects
- IV, difference-in-differences, regression discontinuity
]

--

.pull-left-wide[
Software: `ppi-python` in Python, and the `dsl` package in R. Both wrap around **any** black-box model, with no retraining required.
]
...
---
# .red[Exercise 3: Fixing the Stockholm estimate]
...
|---|---|---|
| Naive | âˆ’0.713 | [âˆ’0.767, âˆ’0.658] |
| Gold standard only | âˆ’0.760 | [âˆ’0.908, âˆ’0.612] |
| Debiased | âˆ’0.814 | [âˆ’0.909, âˆ’0.719] |
| Oracle | âˆ’0.822 | [âˆ’0.871, âˆ’0.774] |
]
...
3. A small **random** sample of hand-coded labels lets you measure that bias and subtract it
4. Validity comes from the **sampling design**; the model only buys precision
...
.pull-left-wide[
**Better models do not remove the need for research design. They make good research design more valuable.**
]
